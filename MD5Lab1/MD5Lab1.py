...
def main():
    ...
    md5Sum = md5Hashing(md5Arr)

    for i in range(4):
        firstByte = (md5Sum[i] & 0x000000ff) << 24  #обращаем порядок байт в словах выходного буфера
        secByte = (md5Sum[i] & 0x0000ff00) << 8
        thirByte = (md5Sum[i] & 0x00ff0000) >> 8
        fourByte = (md5Sum[i] & 0xff000000) >> 24
        md5Sum[i] = firstByte + secByte + thirByte + fourByte

    with open(destinFileName, 'wb') as resFile:
        for i in range(4):
            resFile.write(hex(md5Sum[i])[2:-1])
