...
def sha1Hashing(plainText):
    ...
    resStr = ""
    for i in range(5):
        stringToFile = hex(wordsArr[i])[2:-1]    #take only words from the hex-string, throw unnecessary symbols
...
def main():
    ...
    sha1Sum = sha1Hashing(plainText)      #calculate sha1-hash sum

    with open(destinFileName, 'wb') as resFile:     #write result to the file
        resFile.write(sha1Sum)     
